...
FVector AFGGridActor::GetWorldLocationFromIndex(int32 Index) const
{
	int32 X = Index % Width;
	int32 Y = Index / Width;
...
TArray<int32> AFGGridActor::GetCardinalNeighbours(int32 index) const
{
	TArray<int32> Neighbours;

	for (FVector2D Direction : CardinalDirection)
	{
		if (!IsValidXDirection(index, Direction.X))
			continue;
		int32 NeighbourIndex = index + Direction.X - Direction.Y * Width;
		if (IsTileIndexValid(NeighbourIndex) && !TileList[NeighbourIndex].bBlock)
...
TArray<int32> AFGGridActor::GetDiagonalNeighbours(int32 index) const
{
	TArray<int32> Neighbours;
	
	for (FVector2D Direction : DiagonalDirection)
	{
		int32 NeighbourIndex = index + Direction.X - Direction.Y * Width;
		if (IsTileIndexValid(NeighbourIndex))
			Neighbours.Add(NeighbourIndex);
	}
...
bool AFGGridActor::IsValidXDirection(int32 Index, int32 X) const
{
	if (Index % Width == 0 && X == -1)
		return false;
	if (Index % Width == Width - 1 && X == 1)
		return false;
